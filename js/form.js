...
$('.login').submit(function(e) {
		// Chỉ xử lý form demo (popup), bỏ qua form đăng ký và đăng nhập thật
		var self = $(this);
		var formAction = (self.attr('action') || '').toLowerCase();
		var formMethod = (self.attr('method') || '').toLowerCase();
		
		// Nếu là form đăng ký hoặc đăng nhập thực tế (post tới login/register) thì không chặn submit
		if (
			formAction === 'register' ||
			formAction === 'login' ||
			formAction.indexOf('login_register/register') !== -1 ||
			formAction.indexOf('login_register/login') !== -1 ||
			(formAction.indexOf('login') !== -1 && formMethod === 'post')
		) {
